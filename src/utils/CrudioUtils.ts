...
export default class CrudioUtils {
  static DateDuration(diff: any): Duration {
    ...
    const dr = (source, dest, fname) => {
      ...
      let low = source[`${fname}_lo`] ?? source[fname];
      let high = source[`${fname}_hi`] ?? source[fname];
  ...
  public static TitleCase(input: string): string {
    const converter: any = function (matches: string[]) {
      return matches[1].trim().toUpperCase();
    };

    let result: any = input.replace(/(\-\w)/g, converter);
  ...
  public static GetRandomNumber(min: number, max: number): number {
    const rndValue: number = Math.random();
