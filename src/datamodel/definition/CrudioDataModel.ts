...
export default class CrudioDataModel {
  ...
  private LoadEntityDefinitions(datamodel: ICrudioSchemaDefinition): void {
    ...
    const entity_names: string[] = Object.keys(datamodel.entities);

    for (let index = 0; index < entity_names.length; index++) {
      const entityname: string = entity_names[index];
      const entitySchema: any = datamodel.entities[entityname];
  ...
  private CreateEntityDefinition(entityDefinition: ICrudioEntityDefinition, entityname: string): void {
    const entityType: CrudioEntityDefinition = this.CreateEntityType(entityname, entityDefinition.abstract, false);
    ...
    const fKeys: string[] = Object.keys(entityDefinition.fields ?? {}).filter(f => !this.ignoreFields.includes(f));

    for (let findex = 0; findex < fKeys.length; findex++) {
      const fieldname: string = fKeys[findex];
      const fieldSchema: any = entityDefinition.fields[fieldname];
  ...
  private InheritBaseFields(baseEntityName: string, targetEntity: CrudioEntityDefinition): void {
    const baseEntity: CrudioEntityDefinition = this.GetEntityDefinition(baseEntityName)!;
   ...
   */
  public GetEntityDefinition(entityName: string, failIfNotFound = true): CrudioEntityDefinition | null {
    const matches: CrudioEntityDefinition[] = this.entityDefinitions.filter((e: CrudioEntityDefinition) => e.Name === entityName);
  ...
  private CreateEntityType(name: string, isAbstract, isManyToMany): CrudioEntityDefinition {
    const exists: CrudioEntityDefinition | null = this.GetEntityDefinition(name, false);

    if (exists !== null) {
      throw new Error(`CreateEntityType: '${name}' already exists in model`);
    }

    let entityDefinition: CrudioEntityDefinition | null = this.GetEntityDefinition(name, false);
  ...
  private JoinOneToMany(r: CrudioRelationship): void {
    const sourceTable: CrudioTable = this.GetTableForEntityDefinition(r.FromEntity)!;
    const targetTable: CrudioTable = this.GetTableForEntityDefinition(r.ToEntity)!;
  ...
  private JoinManyToMany(d: CrudioEntityDefinition): void {
    const joinTable: CrudioTable = this.GetTableForEntityDefinition(d.Name);
    const sourceTable: CrudioTable = this.GetTableForEntityDefinition(d.SourceRelationship.FromEntity);
    const targetTable: CrudioTable = this.GetTableForEntityDefinition(d.SourceRelationship.ToEntity);
   ...
   * @private
   */
  private ConnectDefaultRelationships(): void {
  ...
  private JoinNamedRelationships(r: CrudioRelationship): void {
    ...
    enumerated_table.DataRows.map(parent => {
      ...
      if (r.SingularRelationshipValues.length > sourceRows.length)
        throw new Error(`Error: Singular relationship involving Enumerated:${enumerated_table.TableName} Source:${sourceTable.TableName} Target:${targetTable.TableName} - the number of singular values exceeds the number of rows in ${sourceTable.TableName} `);
  ...
  public GetTable(name: string): CrudioTable {
    const matches: CrudioTable[] = this.Tables.filter((t: CrudioTable) => t.TableName === name);
  ...
  public GetTableForEntityDefinition(name: string): CrudioTable {
    const matches: CrudioTable[] = this.Tables.filter((t: CrudioTable) => t.EntityDefinition.Name === name);
  ...
  public static FromJson(config: ICrudioConfig, autoPopulate = true): CrudioDataModel {
    try {
      ...
      console.log("Error loading JSON model");
      console.log(e);
      throw e;
    }
  }
  ...
  public ToString(): string {
    const serialised: string = stringify(this);
  ...
  public ToMermaid(): string {
    let output = "# Entity Relationship Diagram\r\r";
  ...
  GetAllRows(tableName: string): CrudioEntityInstance[] {
    const t: CrudioTable | null = this.GetTable(tableName);
    if (t !== null) {
      return t.DataRows;
    } else {
  ...
  private CreateEntityInstance(entityType: CrudioEntityDefinition): CrudioEntityInstance {
    const entity: CrudioEntityInstance = entityType.CreateInstance();
  ...
  private SetupEntityGenerators(entity: CrudioEntityInstance) {
    entity.EntityDefinition.fields.map(field => {
      const generator: string | undefined = field.fieldOptions.generator;
        ...
        if (typeof field_value === "string" && field_value.indexOf("[") >= 0) {
          ...
          if (entity_field && entity_field.fieldOptions.isUnique) {
            if (entityInstance.EntityDefinition.HasUniqueValue(field_name, detokenised_value)) {
              if (maxtries == 0) {
                throw new Error(`Error: Failed to create unique value for ${entityInstance.EntityDefinition.Name}.${entity_field.fieldName}. Try to define a generator that will create more random values. Adding a random number component may help.`);
              }
  ...
  private ProcessTokensInField(entity: CrudioEntityInstance, fieldName: string, clean: boolean): string {
    let value = entity.DataValues[fieldName];
  ...
  private ReplaceTokens(fieldValue: string, entity: CrudioEntityInstance | null = null): any {
    ...
    do {
      const tokens: string[] | null = fieldValue.match(/\[.*?\]+/g);
      let value: any;
      ...
      tokens.map(t => {
        const token: string = t.replace(/\[|\]/g, "");
        let fieldName: string = token;

        // find parameter characters:
        // ! : get field from context
        // ~ : remove all spaces and convert to lower case
        const params: string[] = fieldName.match(/^\?|!|~|\*/g) || [];
  ...
  public GetGeneratedValue(generator_name: string): any {
    ...
    const generator = this.GetGenerator(generator_name);
    ...
    if (generator && generator.values) {
      ...
      if (typeof generator.values === "object") {
        ...
        const content: string = generator.values ?? generator_name;
  ...
  private ExecuteTrigger(parent_entity: CrudioEntityInstance, trigger: string): void {
    ...
    const rows = this.ExecuteCrudioQuery(entity_type, query);
    if (rows.length == 0) {
    ...
    for (let ri = row_start; ri <= row_end; ri++) {
  ...
  private ConnectChildWithRelatedEntity(parent_entity: CrudioEntityInstance, field_name: string, row_index: number, childs_subobject: CrudioEntityInstance) {
    ...
    if (parent_array.length < row_index + 1) {
      ...
      while (parent_array.length < row_index + 1) {
        const child_tentity = this.CreateEntityInstance(entity_definition);
  ...
  private GetObjectFromPath(path: string): CrudioEntityInstance {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (start > 0) {
        const end = p.indexOf(")");
        const val = p.substring(start, end);
  ...
  public async ExecuteStreams(dataWrapper: ICrudioDataWrapper | null): Promise<void> {
    ...
    for (let i = 0; i < this.DataModel.streams.length; i++) {
  ...
  private async ExecuteStream(name: string, dataWrapper: ICrudioDataWrapper): Promise<void> {
    ...
    for (let c = 0; c < parentTable.DataRows.length; c++) {
