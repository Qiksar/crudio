...
export default class CrudioEntityDefinition implements ICrudioFacet {
  ...
  public get KeyField(): CrudioField | null {
    const fields: CrudioField[] = this.fields.filter(f => f.fieldOptions.isKey);
  ...
  public GetField(fieldName: string, failIfNotFound?: boolean): CrudioField {
    const fields: CrudioField[] = this.fields.filter(f => f.fieldName === fieldName);
  ...
  AddKey(fieldType?: string): CrudioEntityDefinition {
    ...
    const keyField: CrudioField = new CrudioField(this.config.idField, fieldType || "uuid");
