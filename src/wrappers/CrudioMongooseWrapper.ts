...
export default class CrudioMongooseWrapper implements ICrudioDataWrapper {
  ...
  public async PopulateDatabaseTables(): Promise<void> {
    for (let index = 0; index < this.crudio_model.Tables.length; index++) {
   ...
   * @returns {Promise<void>}
   */
  public async InsertTableData(table: CrudioTable): Promise<void> {
  ...
  private async PopulateDatabaseTable(table: CrudioTable): Promise<void> {
    ...
    const model = this.Models[table.TableName];

    for (let r = 0; r < table.DataRows.length; r++) {
      const data = table.DataRows[r].DataValues;
      const values = {};
  ...
  private async AssignOneToManyKeys(): Promise<void> {
    ...
    for (let fi = 0; fi < relationships.length; fi++) {
  ...
  private async AssignManyToManyKeys(): Promise<void> {
    ...
    for (let i = 0; i < this.crudio_model.ManyToManyDefinitions.length; i++) {
  ...
  private async ProcessManyToMany(join_table: CrudioTable, from_entity: string, to_entity: string, from_table: string, to_table: string): Promise<void> {
    ...
    for (let uk = 0; uk < from_unique_keys.length; uk++) {
      ...
      if (record) {
        await record.updateOne({ [to_table]: to_keys }, { new: true });
      } else {
  ...
  private async AssignKeys(parent: string, child: string): Promise<void> {
    ...
    for (let i = 0; i < parent_keys.length; i++) {
      ...
      try {
        ...
        await parent_model.findOneAndUpdate({ [this.config.idField]: parent_id }, { [child]: child_keys });
      } catch (e) {
        console.log(e);
      }
    }
