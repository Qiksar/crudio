...
export default class CrudioMongooseDataModel {
	...
	private CreateSchemaModels() {
		...
		try {
			...
			this.crudio_model.Tables.filter(t => !t.EntityDefinition.IsManyToManyJoin).map(t => {
				...
				schema = this.schema[t.TableName];
				this.models[t.TableName] = model(table_name, schema);
			});
		} catch (e) {
			throw `CreateSchemaModels: error building model for '${table_name}' with schema: ${JSON.stringify(schema)}\r\n Error: ${e}\r\n`;
		}
	}
	...
	private AssignForeignKeys(): void {
		...
		this.crudio_model.ManyToManyRelationships.map(r => {
			...
			if (!from_entity || !to_entity || !from_schema || !to_schema) throw "Invalid many to many relationship definition " + JSON.stringify(r);
